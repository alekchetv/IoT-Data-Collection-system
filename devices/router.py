...
from ino_generator import cpp_generate, json_pin_generate
...
async def add_device(request: Request, device: Device):
    ...
    for sensor in device.sensors:
        # db_sensor_type = await SensorTypeREPO.find_one_or_none(sensor_name=sensor.sensor_type)
        # if not db_sensor_type:
        #     await DeviceREPO.delete_by_mac_address(device.mac_address)
        #     raise HTTPException(status_code=403, detail="Sensor Type does not exist")
...
@router.post("/testcpp")
async def test_json_create(device: Device):
    await json_pin_generate(device)
